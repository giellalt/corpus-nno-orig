...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'https://www.stortinget.no/globalassets/pdf/diverse/mina-og-loven-nynorsk.pdf'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'Mina og løva'"/>
    <!--
...
<xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                <xsl:with-param name="inputString" select="$text"/>
                <xsl:with-param name="target" select="'Ǯ/Â/Ê/Stortingets administrasjon Informasjonsseksjonen Trykk: Stortingets hustrykkeri 2013 ISBN 978-82-8196-076-3/'"/>
                <xsl:with-param name="replacement" select="'ø/å/æ//'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
        </xsl:element>
    </xsl:template>
